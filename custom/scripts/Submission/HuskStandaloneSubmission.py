...
def GetSettingsFilename():
    return os.path.join(GetDeadlineSettingsPath(), "CommandLineSettings.ini")
...
def SubmissionDialog():
    ...
    #file dialog
    scriptDialog.AddControlToGrid("InputLabel", "LabelControl", "USD File", 4, 0, "The USD file you wish to render.",
                                  False)
    filesToProcess = scriptDialog.AddSelectionControlToGrid("USDFilePath", "MultiFileBrowserControl", "",
                                                            "USD Files (*.usd);;USDA Files (*.usda);;USDC Files(*.usdc);;USDZ Files(*.usdz)",
                                                            4, 1, colSpan=5)
    #Frame Boxes
...
def SubmitButtonPressed():
    ...
    #Create Job file
    jobInfoFilename = os.path.join(ClientUtils.GetDeadlineTempPath(), "usd_job_info.job")
    ...
    pluginInfoFilename = os.path.join(GetDeadlineTempPath(), "USD_plugin_info.job")
